...
def setup_mlflow():
    ...
    if token:
        ...
        print("Tracking MLflow using local storage.")

    try:
        mlflow.set_experiment("CI_Retraining_Obesity")
    except Exception as e:
        print(f"Warning: Failed to set experiment ({e}). Falling back to local storage.")
        mlflow.set_tracking_uri("sqlite:///mlflow.db")
        mlflow.set_experiment("CI_Retraining_Obesity")
...
def main():
    ...
    with mlflow.start_run(run_name="CI_Automated_Retraining") as run:
        ...
        print("Training Random Forest model (CI Retraining)...")
        ...
        print(f"Retraining completed cleanly. Run ID: {run_id} | Accuracy: {acc:.4f}")
